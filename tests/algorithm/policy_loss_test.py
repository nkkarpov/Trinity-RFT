...
class VerlPolicyLossTest(unittest.TestCase):
    ...
    def test_gspo_policy_loss(self):
        ...
        policy_loss_fn = policy_loss_fn_cls(**policy_loss_fn_args)
        loss, metrics = policy_loss_fn(log_prob=self.logprob, **self.input_data.batch)
        gspo_loss_expected = torch.tensor(0.15352888405323029)
        pg_clipfrac_expected = torch.tensor(0.4791666567325592)
        ppo_kl_expected = torch.tensor(-0.021903187036514282)
        print("gspo_loss_expected:", loss.item())
        print("pg_clipfrac_expected:", metrics["pg_clipfrac"])
        print("ppo_kl_expected:", metrics["ppo_kl"])
